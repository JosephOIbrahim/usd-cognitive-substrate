...
\begin{figure}[htbp]
    ...
    \caption{Determinism Boundary in AI-Human Interaction. Human agency
             ...
             irreducible. With ThinkingMachines batch-invariant kernels,
             the entire computational pipeline becomes deterministic:
             signal detection, routing, expert selection, LLM generation,
             and state updates all guarantee that
             \textit{Same input + Same state $\rightarrow$ Same output + Same state update}.}
    \label{fig:determinism-boundary}
\end{figure}
